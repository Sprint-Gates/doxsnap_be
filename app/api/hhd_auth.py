...
from app.models import HandHeldDevice, Technician, Company, handheld_device_technicians
...
async def hhd_login(
    ...
    # Check if device has assigned technician(s)
    if not device.assigned_technician and len(device.assigned_technicians) == 0:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Device has no assigned technicians"
        )
